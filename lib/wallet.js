...
class Wallet {
    constructor(hostname, port, user, pass) {
        ...
        this.port = port || 21065;
        this.user = user || '';
        this.pass = pass || '';

        this._request('get_balance'); // This line is necessary in order to do the initial handshake between this wrapper and uplexa-wallet-rpc; without it, the first request to the wrapper fails (subsequent request succeed, though.)
    }
}
...
Wallet.prototype.create_wallet = function(filename, password, language) {
    ...
    let params = {
        filename: filename || 'uplexa_wallet',
        'password': password || '',
...
// transfer uPlexa to a single recipient, or a group of recipients
...
// helper function to convert uPlexa amount to atomic units
